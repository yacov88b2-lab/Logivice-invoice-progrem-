...
function Setup2FAPanel({ onClose, onSetupDone }: { onClose: () => void; onSetupDone: () => void }) {
  ...
  return (
    <Modal title="Enable Two-Factor Authentication" onClose={onClose}>
      ...
      <div className="flex gap-2">
        <button onClick={onSetupDone} className="flex-1 rounded-lg bg-[#1e3a8a] px-3 py-2 text-sm font-semibold text-white hover:bg-[#1f1d70]">Continue</button>
  ...
  if (backupCodes) {
    return (
      <Modal title="Save your backup codes" onClose={onClose}>
        ...
        <button onClick={onClose} className="w-full rounded-lg bg-[#1e3a8a] px-3 py-2 text-sm font-semibold text-white hover:bg-[#1f1d70]">
    ...
    <Modal title="Confirm 2FA setup" onClose={onClose}>
      ...
      <form onSubmit={handleConfirm} className="space-y-3">
        <input
          ...
          maxLength={6}
          required
          className="w-full rounded border border-slate-300 px-3 py-2 text-sm text-center tracking-widest font-mono focus:border-[#1e3a8a] focus:outline-none"
        />
        {error && <p className="text-sm text-red-600">{error}</p>}
        <div className="flex gap-2">
          <button type="submit" disabled={loading || code.length < 6} className="flex-1 rounded-lg bg-[#1e3a8a] px-3 py-2 text-sm font-semibold text-white hover:bg-[#1f1d70] disabled:opacity-50">
  ...
  if (newCodes) {
    return (
      <Modal title="New backup codes" onClose={onClose}>
        ...
        <button onClick={onClose} className="w-full rounded-lg bg-[#1e3a8a] px-3 py-2 text-sm font-semibold text-white hover:bg-[#1f1d70]">Done</button>
      </Modal>
    );
  }
  ...
  return (
    <Modal title="Regenerate backup codes" onClose={onClose}>
      ...
      <form onSubmit={handleRegenerate} className="space-y-3">
        ...
        {error && <p className="text-sm text-red-600">{error}</p>}
        <div className="flex gap-2">
          <button type="submit" disabled={loading} className="flex-1 rounded-lg bg-[#1e3a8a] px-3 py-2 text-sm font-semibold text-white hover:bg-[#1f1d70] disabled:opacity-50">
